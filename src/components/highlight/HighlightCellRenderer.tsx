...
const filterHighLightCols: any = [];

const HighlightCellRenderer = (props: any) => {
  const context = React.useContext(SearchContext);
  const columnName = props.colDef.field;
  let filterObj = props.api.getFilterModel();
  let value: any;
  filterObj =
    filterHighLightCols.includes(columnName) && filterObj[columnName]
      ? filterObj[columnName]
      : null;
  value = typeof value === "string" ? props.value.trim() : props.value;
  let searchString = [];
  if (filterObj) {
    if (filterObj.filter) {
      ...
      return valueArray.map((v, i) => (
        <React.Fragment key={i}>
          ...
          {i + 1 !== valueArray.length && (
            <span className="table-text-highlight">{matches[i]}</span>
          )}
        </React.Fragment>
      ));
    }
